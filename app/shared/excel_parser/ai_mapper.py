"""
app/shared/excel_parser/ai_mapper.py
────────────────────────────────────
AI-powered Excel column mapper.

When static alias matching fails to resolve one or more required fields, this
module asks the LLM (Claude → Groq fallback via LLMClient) to inspect the raw
column headers and a few sample data rows, then return a JSON mapping of
...
Canonical field names understood by the parser:
    ...
    problem_solving     – developer critical-thinking / PS score     (0-10)
    ...
    gitlab_username     – GitLab handle (optional)
    team_name           – team label (optional)
...
which column corresponds to each of the following canonical fields:
  ...
  problem_solving    – Developer critical thinking / problem solving score (numeric, range 0-10)
  ...
  gitlab_username    – GitLab handle / user name (optional)
  team_name          – Team label / department (optional)
...
Example response:
{
  ...
  "Critical Thinking (0-10)": "problem_solving",
  "Annual KPI (15%)": "kpi",
