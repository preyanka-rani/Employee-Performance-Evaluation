...
_REPORTS_DIR = pathlib.Path("outputs") / "application"
...
async def generate_application_excel_report(
    run_id: int,
    emails: list[str],
    team: str,
    year: int,
    ...
    tl_readiness_hdr = _cn.get("support_readiness", _DEFAULT_COL_NAMES["support_readiness"])
    ...
    detail_headers = [
        ...
        tl_readiness_hdr, tl_kpi_hdr, tl_general_hdr,
    ...
    detail_widths = [
        ...
        18, 22, 22, 28, 22,
        22, 20, max(24, len(tl_readiness_hdr) + 2), max(20, len(tl_kpi_hdr) + 2),
    ...
    if scores_sorted:
        ...
        summary[11] = round(sum(r["attendance_score"] for r in scores_sorted) / len(scores_sorted), 2)
        summary[12] = round(sum(r["attendance_marks"] for r in scores_sorted) / len(scores_sorted), 2)
        summary[13] = round(sum(r["support_readiness"] for r in scores_sorted) / len(scores_sorted), 2)
        summary[14] = round(sum(r["kpi"] for r in scores_sorted) / len(scores_sorted), 2)
        summary[15] = round(sum(r["general"] for r in scores_sorted) / len(scores_sorted), 2)
        summary[16] = round(sum(r["tl_total"] for r in scores_sorted) / len(scores_sorted), 2)
    ...
    summary_headers = [
        "Team", "Employee", "Email",
        "avg_functional_score", tl_readiness_hdr,
    ...
    summary_col_widths = [
        ...
        max(24, len(tl_readiness_hdr) + 2), 26,
    ...
    for rec in scores_sorted:
        ...
        avg_eval = round(
            rec["segment_a_marks"] + rec["support_readiness"]
    ...
    if scores_sorted:
        ws2.append([])
        avg_seg_a = round(sum(r["segment_a_marks"] for r in scores_sorted) / len(scores_sorted), 2)
        avg_readiness = round(sum(r["support_readiness"] for r in scores_sorted) / len(scores_sorted), 2)
        ...
        avg_eval_all = round(avg_seg_a + avg_readiness + avg_attendance + avg_kpi + avg_general, 2)
    ...
    filename = f"Application_Final_Report_{team}_{year}_{month:02d}.xlsx"
    output_path = _REPORTS_DIR / filename
    wb.save(str(output_path))

    logger.info("application_report_saved", path=str(output_path), employee_count=len(scores_sorted))
