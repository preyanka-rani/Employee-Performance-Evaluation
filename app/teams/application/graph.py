...
class ApplicationWorkerState(TypedDict, total=False):
    employee_id: str
    employee_email: str
    employee_name: str
    year: int
...
async def fetch_crm_node(state: ApplicationWorkerState) -> dict:
    log = logger.bind(employee_email=state["employee_email"])
    log.info("application_fetch_crm_start")
    ...
    log.info("application_fetch_crm_done", log_records=len(log_records), descriptions=len(descriptions))
    return {"crm_log_records": log_records, "crm_description_records": descriptions}


async def fetch_hr_node(state: ApplicationWorkerState) -> dict:
    log = logger.bind(employee_email=state["employee_email"])
    log.info("application_fetch_hr_start")
    ...
    log.info("application_fetch_hr_done", attendance_records=len(attendance_records))
    return {"attendance_records": attendance_records}


async def load_tl_node(state: ApplicationWorkerState) -> dict:
    log = logger.bind(employee_email=state["employee_email"])
    ...
    log.info("tl_loaded", support_readiness=support_readiness, kpi=kpi, general=general)
    return {
...
async def compute_functional_node(state: ApplicationWorkerState) -> dict:
    log = logger.bind(employee_email=state["employee_email"])
    ...
    from app.teams.application.formulas import normalise_application_log_hours
    log_hours_score = normalise_application_log_hours(total_hours)
    ...
    monthly_functional_score = compute_application_functional_score(
    ...
    log.info(
        "application_functional_computed",
...
async def compute_segment_b_node(state: ApplicationWorkerState) -> dict:
    log = logger.bind(employee_email=state["employee_email"])
    ...
    log.info(
        "application_segment_b_computed",
...
async def compute_final_node(state: ApplicationWorkerState) -> dict:
    log = logger.bind(employee_email=state["employee_email"])

    base_total, final_score = compute_application_final_score(
        segment_a_marks=state["segment_a_marks"],
        segment_b_marks=state["segment_b_marks"],
    )

    log.info("application_final_computed", base_total=base_total, final_score=final_score)
    return {"base_total": base_total, "final_score": final_score}


def build_application_graph() -> Any:
    builder = StateGraph(ApplicationWorkerState)
...
async def run_application_worker(
    *,
    employee_id: str,
    employee_email: str,
    employee_name: str,
    ...
    month: int,
    db: AsyncSession,
) -> ApplicationWorkerState:
    initial_state: ApplicationWorkerState = {
    ...
    final_state: ApplicationWorkerState = await _application_graph.ainvoke(initial_state)
    return final_state
