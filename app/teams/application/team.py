...
class ApplicationTeam(TeamContract):
    team_key: ClassVar[str] = "application"
    display_name: ClassVar[str] = "Application"
    aliases: ClassVar[frozenset[str]] = frozenset(
        {"application", "application_team", "app"}
    )
    ...
    async def run_per_employee(
        ...
        log.info("application_team_run_start")
        ...
        try:
            state = await run_application_worker(
            ...
            log.error("application_worker_graph_failed", error=str(exc))
            result["error"] = str(exc)
            return result

        if state.get("fetch_error"):
            log.error("application_worker_fetch_error", error=state["fetch_error"])
        ...
        log.info("application_team_run_done", final_score=state["final_score"])
        return result
    ...
    async def generate_report(
        ...
        **kwargs: Any,
        ...
        return await generate_application_excel_report(
            run_id=run_id,
            emails=emails,
            team=team_key,
            year=year,
