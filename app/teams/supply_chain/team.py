...
from app.teams.supply_chain.graph import run_supply_chain_worker
from app.teams.supply_chain.report import generate_supply_chain_excel_report

logger = get_logger(__name__)


class SupplyChainTeam(TeamContract):
    team_key: ClassVar[str] = "supply_chain"
    display_name: ClassVar[str] = "Supply Chain"
    ...
    async def run_per_employee(
        ...
        log.info("supply_chain_team_run_start")
        ...
        try:
            state = await run_supply_chain_worker(
            ...
            log.error("supply_chain_worker_graph_failed", error=str(exc))
            result["error"] = str(exc)
            return result

        if state.get("fetch_error"):
            log.error("supply_chain_worker_fetch_error", error=state["fetch_error"])
        ...
        log.info("supply_chain_team_run_done", final_score=state["final_score"])
        return result
    ...
    async def generate_report(
        ...
        **kwargs: Any,
        ...
        return await generate_supply_chain_excel_report(
            run_id=run_id,
            emails=emails,
            team=team_key,
            year=year,
