...
from app.teams.hr.formulas import (
    compute_attendance_marks,
    compute_hr_final_score,
    compute_hr_functional_score,
...
class HRWorkerState(TypedDict, total=False):
    employee_id: str
    employee_email: str
    employee_name: str
    year: int
...
async def fetch_crm_node(state: HRWorkerState) -> dict:
    log = logger.bind(employee_email=state["employee_email"])
    log.info("hr_fetch_crm_start")
    ...
    log.info("hr_fetch_crm_done", log_records=len(log_records), descriptions=len(descriptions))
    return {"crm_log_records": log_records, "crm_description_records": descriptions}


async def fetch_hr_node(state: HRWorkerState) -> dict:
    log = logger.bind(employee_email=state["employee_email"])
    log.info("hr_fetch_attendance_start")
    ...
    log.info("hr_fetch_attendance_done", attendance_records=len(attendance_records))
    return {"attendance_records": attendance_records}


async def load_tl_node(state: HRWorkerState) -> dict:
    log = logger.bind(employee_email=state["employee_email"])
...
async def compute_functional_node(state: HRWorkerState) -> dict:
    log = logger.bind(employee_email=state["employee_email"])
    ...
    log_hours_score, monthly_functional_score = compute_hr_functional_score(
...
async def compute_segment_b_node(state: HRWorkerState) -> dict:
    log = logger.bind(employee_email=state["employee_email"])
...
async def compute_final_node(state: HRWorkerState) -> dict:
    log = logger.bind(employee_email=state["employee_email"])

    base_total, final_score = compute_hr_final_score(
        segment_a_marks=state["segment_a_marks"],
        segment_b_marks=state["segment_b_marks"],
    )

    log.info("hr_final_computed", base_total=base_total, final_score=final_score)
...
async def run_hr_worker(
    *,
    ...
    final_state: HRWorkerState = await _hr_graph.ainvoke(initial_state)
    return final_state
