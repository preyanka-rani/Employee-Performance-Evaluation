...
_REPORTS_DIR = pathlib.Path("outputs") / "hr"

_DEFAULT_COL_NAMES: dict[str, str] = {
    "team_name": "team_name",
    "problem_solving": "Talent Acquisition & HR Operations (0-10)",
...
async def generate_hr_excel_report(
    run_id: int,
    emails: list[str],
    team: str,
    year: int,
    ...
    detail_widths = [
        ...
        18, 22, 22, 22, 28, 22,
        22, 20, max(38, len(tl_ps_hdr) + 2), max(20, len(tl_kpi_hdr) + 2),
    ...
    filename = f"HR_Final_Report_{team}_{year}_{month:02d}.xlsx"
    output_path = _REPORTS_DIR / filename
    wb.save(str(output_path))

    logger.info("hr_report_saved", path=str(output_path), employee_count=len(scores_sorted))
