...
from app.teams.finance.graph import run_finance_worker
from app.teams.finance.report import generate_finance_excel_report

logger = get_logger(__name__)


class FinanceTeam(TeamContract):
    team_key: ClassVar[str] = "finance"
    display_name: ClassVar[str] = "Finance"
    aliases: ClassVar[frozenset[str]] = frozenset(
        {"finance", "finance_team", "fin"}
    )
    ...
    async def run_per_employee(
        ...
        log.info("finance_team_run_start")
        ...
        try:
            state = await run_finance_worker(
            ...
            log.error("finance_worker_graph_failed", error=str(exc))
            result["error"] = str(exc)
            return result

        if state.get("fetch_error"):
            log.error("finance_worker_fetch_error", error=state["fetch_error"])
        ...
        log.info("finance_team_run_done", final_score=state["final_score"])
        return result
    ...
    async def generate_report(
        ...
        **kwargs: Any,
        ...
        return await generate_finance_excel_report(
            run_id=run_id,
            emails=emails,
            team=team_key,
            year=year,
