...
def normalise_finance_log_hours(hours: float) -> float:
    if hours >= 160:
        return 100.0
    elif hours >= 140:
        return 80.0
...
def compute_finance_functional_score(
    log_hours: float,
    sentiment_score: float,
) -> tuple[float, float]:
    log_hours_score = normalise_finance_log_hours(log_hours)
...
def compute_finance_final_score(
