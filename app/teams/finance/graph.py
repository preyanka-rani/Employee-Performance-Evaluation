...
from app.teams.finance.formulas import (
    compute_attendance_marks,
    compute_finance_final_score,
    compute_finance_functional_score,
...
class FinanceWorkerState(TypedDict, total=False):
    employee_id: str
    employee_email: str
    employee_name: str
    year: int
...
async def fetch_crm_node(state: FinanceWorkerState) -> dict:
    log = logger.bind(employee_email=state["employee_email"])
    log.info("finance_fetch_crm_start")
    ...
    log.info("finance_fetch_crm_done", log_records=len(log_records), descriptions=len(descriptions))
    return {"crm_log_records": log_records, "crm_description_records": descriptions}


async def fetch_hr_node(state: FinanceWorkerState) -> dict:
    log = logger.bind(employee_email=state["employee_email"])
    log.info("finance_fetch_hr_start")
    ...
    log.info("finance_fetch_hr_done", attendance_records=len(attendance_records))
    return {"attendance_records": attendance_records}


async def load_tl_node(state: FinanceWorkerState) -> dict:
    log = logger.bind(employee_email=state["employee_email"])
...
async def compute_functional_node(state: FinanceWorkerState) -> dict:
    log = logger.bind(employee_email=state["employee_email"])
    ...
    log_hours_score, monthly_functional_score = compute_finance_functional_score(
...
async def compute_segment_b_node(state: FinanceWorkerState) -> dict:
    log = logger.bind(employee_email=state["employee_email"])
...
async def compute_final_node(state: FinanceWorkerState) -> dict:
    log = logger.bind(employee_email=state["employee_email"])

    base_total, final_score = compute_finance_final_score(
        segment_a_marks=state["segment_a_marks"],
        segment_b_marks=state["segment_b_marks"],
    )

    log.info("finance_final_computed", base_total=base_total, final_score=final_score)
    return {"base_total": base_total, "final_score": final_score}


def build_finance_graph() -> Any:
    builder = StateGraph(FinanceWorkerState)
...
async def run_finance_worker(
    *,
    ...
    final_state: FinanceWorkerState = await _finance_graph.ainvoke(initial_state)
    return final_state
