"""
app/teams/hajj_helpdesk/report.py
──────────────────────────────────
Generates formatted Excel reports for Hajj Helpdesk Team evaluation runs.
...
Sheet 2 – "summary_grade" (matches EBS of Business Automation Ltd. format)
    Team | Employee | Email |
...
Saved to: outputs/hajj_helpdesk/Hajj_Helpdesk_Final_Report_{year}_{month:02d}.xlsx
"""
...
# Output directory
_REPORTS_DIR = pathlib.Path("outputs") / "hajj_helpdesk"
...
async def generate_hajj_helpdesk_excel_report(
    run_id: int,
    emails: list[str],
    team: str,
    year: int,
    ...
    """
    Build a formatted two-sheet Excel report for a completed Hajj Helpdesk team
    bulk evaluation run.

    Sheet 1: Full detailed breakdown for every employee.
    Sheet 2: summary_grade (EBS-format final summary).
    ...
    # ═════════════════════════════════════════════════════════════════════════
    # SHEET 2 – summary_grade (EBS of Business Automation Ltd. format)
    #
    # Columns (11):
    #   Team | Employee | Email |
    #   avg_functional_score |
    ...
    ws2 = wb.create_sheet("summary_grade")
    ...
    for s in scores_sorted:
        ...
        # Left-align identification columns (Team, Employee, Email)
    ...
    if scores_sorted:
        ...
        for col_idx in range(1, len(summary_headers) + 1):
            ...
            c.fill = (
                PatternFill("solid", fgColor="1F4E78") if col_idx == 1 else section_fill
            )
            c.border = thin_border
            c.alignment = center_align
    ...
    filename = f"Hajj_Helpdesk_Final_Report_{year}_{month:02d}.xlsx"
    output_path = _REPORTS_DIR / filename
    wb.save(str(output_path))

    logger.info(
        "hajj_helpdesk_report_saved",
